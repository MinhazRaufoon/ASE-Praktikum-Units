...
IRQ_HANDLER(SPC5_ADC0_EOC_HANDLER) {
  ...
  /* Clearing EOC and ECH bits.*/
  ADCD1.adc_tagp->ISR.R = 3U;
...
IRQ_HANDLER(SPC5_ADC1_EOC_HANDLER) {
  ...
  /* Clearing EOC and ECH status bits.*/
  ADCD2.adc_tagp->ISR.R = 3U;
...
void adc_lld_stop(ADCDriver *adcp) {
  ...
  if (&ADCD1 == adcp) {
    ...
    edmaChannelRelease(&adc_adc0_dma_config);
  }
#endif /* SPC5_ADC_USE_ADC0 */
#if (SPC5_ADC_USE_ADC1 == TRUE)
  if (&ADCD2 == adcp) {
    /* Releases the allocated DMA channel.*/
    edmaChannelRelease(&adc_adc1_dma_config);
  }
#endif /* SPC5_ADC_USE_ADC1 */
#endif /* SPC5_ADC_DMA_MODE */
