...
static void icu_lld_serve_interrupt(ICUDriver *icup) {
  ...
  if ((sr & 0x0008U) != 0U) { /* TOF */
    icup->etimerp->CHANNEL[icup->smod_number].STS.R = 0x0008U;  /* Reset TOF bit */
    _icu_isr_invoke_overflow_cb(icup);
  }
  if ((sr & 0x0040U) != 0U) { /* ICF1 */
    if (icup->etimerp->CHANNEL[icup->smod_number].CTRL.B.CNTMODE ==
        SPC5_ETIMER_CNTMODE_RFE_SIHA) {
      icup->etimerp->CHANNEL[icup->smod_number].STS.R = 0x0040U;  /* Reset ICF1 bit */
    ...
    if (icup->etimerp->CHANNEL[icup->smod_number].CTRL.B.CNTMODE ==
        SPC5_ETIMER_CNTMODE_RFE_SIHA) {
      icup->etimerp->CHANNEL[icup->smod_number].STS.R = 0x0080U;  /* Reset ICF2 bit */
